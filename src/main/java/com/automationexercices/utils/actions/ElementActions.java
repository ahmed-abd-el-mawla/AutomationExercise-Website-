    ...
    public void removeAds() {
        try {
            ((JavascriptExecutor) driver).executeScript(
                    "var ads = document.querySelectorAll('iframe[id^=\"aswift\"], div[id^=\"dismiss\"], div[id^=\"google_ads\"], #aswift_0_host');" +
                            "for (var i = 0; i < ads.length; i++) { ads[i].remove(); }"
            );
            LogsManager.info("Ads and Overlays removed successfully from DOM.");
        } catch (Exception e) {
            return;
        }
    }
        ...
        waitManager.fluentWait().until(d ->
                {
                    try {
                        ...
                        try {
                            ...
                            LogsManager.info("Clicked on element via JS: " + locator);
                            return true;
        ...
        waitManager.fluentWait().until(d ->
                {
                    try {
                        ...
                        try {
                            ...
                            LogsManager.info("Typed text via JS into element: " + locator);
                            return true;
                        } catch (Exception jsEx) {
                            return false;
                        }
